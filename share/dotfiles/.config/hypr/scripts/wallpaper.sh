...
# Create the Hyprland template if it doesn't exist
...
$color13 = {color13}
$color14 = {color14}
$color15 = {color15}' > "$HOME/.config/wal/templates/colors-hyprland.conf"
    echo ":: Created Hyprland color template"
fi

# Run pywal with template support
wal -q -i "$used_wallpaper" -t
source "$HOME/.cache/wal/colors.sh"

# Ensure the Hyprland colors file exists
if [ ! -f "$HOME/.cache/wal/colors-hyprland.conf" ]; then
    echo ":: Warning: colors-hyprland.conf not created, creating manually..."
    echo "# Auto generated color theme for Hyprland
\$wallpaper = $used_wallpaper
...
\$color13 = $color13
\$color14 = $color14
\$color15 = $color15" > "$HOME/.cache/wal/colors-hyprland.conf"
fi
